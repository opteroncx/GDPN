...
class SRGAN(BasicRestorer):
    ...
    def train_step(self, data_batch, optimizer):
        ...
        Flag_diff = False
        if Flag_diff:
            fake_g_output,skip = self.generator(lq)
            lq_upsampled = F.interpolate(lq, (1024,1024), mode='bicubic')
            diff = gt-lq_upsampled              # stylegan需要拟合的差值
            # print(gt)
            # print(lq_upsampled)
        else:
        ...
        if (self.step_counter % self.disc_steps == 0
            ...
            if self.pixel_loss:
                ...
                if Flag_diff:
                    losses['loss_pix_diff'] = self.pixel_loss(skip, diff)
            if self.gradient_loss:
                losses['loss_grad'] = self.gradient_loss(fake_g_output, gt)
                if Flag_diff:
                    losses['loss_grad_diff'] = self.gradient_loss(skip, diff)
